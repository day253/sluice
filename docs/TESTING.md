...
| CTRL-003 | Pod/5-0 membership Ready 后，旧 Snapshot 的 retained control 不得继续保留 100 Worker/allocation；目标集群 DNS 返回 fake IP 时 Worker 必须经 K8s API 获取 Service ClusterIP；部署必须验 FSM 角色/容量而非只验 health | `pkg/raft.TestSetControlNodesRemovesLegacyCapacityWithoutChangingLivenessOrInflight`；`pkg/node.TestControlNodesNeedingMigrationUsesOnlyRaftMembersInStableOrder`；`charts/sluice/charttest.TestWorkerEntrypointUsesStableServiceIPInsteadOfClusterDNS` | `test/integration.TestStatelessWorkerRoleSplit`（注入 legacy control 容量/allocation→Leader 切换→5/0 control 镜像收敛→Worker 排空）；`scripts/deploy-remote.sh` 真实 MicroK8s 自动断言 5 control/50 Worker/5000 容量/allocation owner |
