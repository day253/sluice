...
| UI-004 | 四张 174 点图必须可用鼠标读取某个时间点；提示显示时间桶、最近系列和原始值，Worker 图同时显示 Limit；50 实例不能把全部系列塞入提示框；交互只读且不得增加请求或历史存储 | `pkg/webui.TestDashboardChartsExposeNearestPointTooltip` | `test/integration.TestPerformanceDiagnosticsProxyFromFollower`（真实 3 节点 Follower HTTP 交付 tooltip 脚本及 Leader 历史）；远程 50 实例浏览器 pointer hover、tooltip 内容和零 console error 回归 |
