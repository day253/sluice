...
| UI-002 | WebUI 每秒轮询不能重复拉取 `/metrics` 已有的 174 点性能历史；页面只取 Leader 当前快照并复用 metrics 历史，同时默认 performance JSON 必须继续完整可用 | `pkg/metrics.TestCollectorStoresBoundedPerformanceHistory`（完整/当前两种读取）；`pkg/api.TestPerformanceEndpointReturnsConfiguredLeaderDiagnostics`、`TestPerformanceEndpointCanReturnCurrentSnapshotWithoutHistory`；`pkg/webui.TestDashboardIncludesPerformanceVisualizationAndJSONLink` | `test/integration.TestPerformanceDiagnosticsProxyFromFollower`（真实 3 节点、Follower 代理完整 JSON 与 `history=0`、Dashboard 生产脚本）；远程 50 实例同形状 2 万任务开/关页面 A/B |
...
- 执行副本扩容不扩大 voter 多数派；迁移后的 Leader 位于目标 voter 集合，non-voter
